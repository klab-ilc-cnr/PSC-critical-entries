...
var peculiarEntriesQueriesLabel = [
	...
	'Number of PUs connected to MUs connected to SynU Thamus',
	...
	'SynU Thamus with a valid descriptionL',
	'SynU Thamus with a valid framesetL',
	'Number of SynU Thamus with the comment inside the GINP',
	...
	'Number of USYNUSEM between Thamus with NULL idCorresp',
	'Number of USYNUSEM between Thamus entries with idCorresp = 70',
...
var peculiarEntriesQueries = [
	...
	'select count(*) as count from usem where BINARY idUsem REGEXP BINARY \\\'^USemTH\\\'',
	'select count(*) as count from usyns where BINARY idUsyn REGEXP BINARY \\\'^SYNUTH\\\'',
	'select count(distinct idUms) as count from usyns where BINARY idUsyn REGEXP BINARY \\\'^SYNUTH\\\'',
	'select count(*) as count from mus m where m.idMus in (select u.idUms from usyns u where BINARY u.idUsyn REGEXP BINARY \\\'^SYNUTH\\\') and m.ginp is NULL',
	'select count(distinct idPhu) as count  from musphu where idMus in (select m.idMus from mus m where m.idMus in (select u.idUms from usyns u where BINARY u.idUsyn REGEXP BINARY \\\'^SYNUTH\\\') )',
	'select count(*) as count from usem u where BINARY u.idUsem REGEXP BINARY \\\'^USemTH\\\' and u.comment is NULL',
	'select count(*) as count from usem u where BINARY u.idUsem REGEXP BINARY \\\'^USemTH\\\' and u.exemple is NULL',
	'select count(*) as count from usem u where BINARY u.idUsem REGEXP BINARY \\\'^USemTH\\\' and u.definition is NULL',
	'select count(*) as count from usem u where BINARY u.idUsem REGEXP BINARY \\\'^USemTH\\\' and u.pos = \\\'ADV\\\' and u.naming REGEXP \\\'mente$\\\'',
	'select count(*) as count from usem u where BINARY u.idUsem REGEXP BINARY \\\'^USemTH\\\' and u.pos = \\\'NP\\\'',
	...
	'select count(idUsyn) as count from usyns u where BINARY u.idUsyn REGEXP BINARY \\\'^SYNUTH\\\' and BINARY u.comment REGEXP BINARY \\\'^GINP\\\\\\\\d+$\\\'',
	'select count(DISTINCT idUsem) as count from usemtemplates  where BINARY idUsem REGEXP BINARY \\\'^USemTH\\\'',
	'select idUsem, count(*) as count from usemtemplates where BINARY idUsem REGEXP BINARY \\\'^USemTH\\\' group by idUsem  HAVING count(*) = 2',
	'select count(idUsem) as count from usemrel where BINARY idUsem REGEXP BINARY \\\'USemTH\\\' and idRSem = \\\'R146\\\'',
	'select count(idUsem) as count from usemrel where BINARY idUsem REGEXP BINARY \\\'^USemTH\\\' and BINARY idUsemTarget  REGEXP BINARY \\\'^USemTH\\\'',
	'select count(*) as count from usynusem where BINARY idUsyn REGEXP BINARY \\\'^SYNUTH\\\' and BINARY idUsem REGEXP BINARY \\\'^USemTH\\\' and idCorresp is NULL',
	'select count(*) as count from usynusem where BINARY idUsyn REGEXP BINARY \\\'^SYNUTH\\\' and BINARY idUsem REGEXP BINARY \\\'^USemTH\\\' and idCorresp = 70',
